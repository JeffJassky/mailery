...
function AreaChart({ a, b, height = 200 }) {
  const w = 720, h = height;
  const max = Math.max(...a, ...b);
  const x = i => (i / (a.length - 1)) * (w - 32) + 24;
  const y = v => h - 28 - (v / max) * (h - 52);
  const lineA = a.map((v, i) => `${i ? "L" : "M"}${x(i)},${y(v)}`).join(" ");
  const lineB = b.map((v, i) => `${i ? "L" : "M"}${x(i)},${y(v)}`).join(" ");
  const areaA = `M${x(0)},${h-28} ${a.map((v,i)=>`L${x(i)},${y(v)}`).join(" ")} L${x(a.length-1)},${h-28} Z`;
  return (
