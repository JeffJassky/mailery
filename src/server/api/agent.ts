...
export function createAgentRouter(mailer: Mailer, opts: AgentRouterOptions): Router {
  ...
  router.post(
    ...
    wrap(async (req, res) => {
      ...
      const b = await createBroadcast(mailer, parsed.data as any, actorOf(req), { requireSubscribed: true, strictSegment: true })
  ...
  router.patch(
    ...
    wrap(async (req, res) => {
      ...
      if (!parsed.success) return res.status(400).json({ error: 'validation_failed', message: zodMessage(parsed.error) })
      const b = await patchBroadcast(mailer, String(req.params.slug), parsed.data as any, actorOf(req), { requireSubscribed: true, strictSegment: true })
  ...
  /**
   * The true recipient count — the stream dispatch consumes, counted: host
   * filter, mailer-side post-filters, sendable address, suppression for the
   * template's kind, minus contacts that already have a send row for this
   * broadcast. `recipientCount` is the number schedule requires as
   * `confirmedCount`.
   */
  router.post(
  ...
  /**
   * Re-open a paused broadcast. The next wave of a capped broadcast:
   * `{recipientCap: <higher>, confirmedCount}` where confirmedCount is
   * `recipientCount` from POST /broadcasts/:slug/count after the cap change —
   * i.e. count first with the cap you intend (the count route reports
   * `uncappedRecipientCount` and `sendsSoFar`, so the next slice is
   * `min(uncapped, newCap - sendsSoFar)`).
   */
  router.post(
